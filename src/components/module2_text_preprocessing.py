...
# Load spaCy model for entity extraction - always attempt to load
nlp = None
...
def segment_clauses(text: str) -> list:
    ...
    # First, try to split on clear markers (even if embedded in text)
    markers = [
    ...
    if not splits:
        # No markers found - fallback to paragraph split
    ...
    for i, (pos, label, matched_text) in enumerate(splits):
        if i == 0 and pos > 0:
            # Add anything before first marker
...
def extract_entities(text: str) -> list:
    if nlp is None:
        # spaCy model not loaded - return empty list
        return []
