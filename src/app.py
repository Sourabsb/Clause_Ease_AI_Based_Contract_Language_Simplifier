...
class Document(Base):
    ...
    report_json = Column(Text)  # Processing results
    stats_json = Column(Text)  # Statistics data
...
def process_document():
    ...
    try:
        ...
        for idx, clause_data in enumerate(processed_clauses):
            ...
            simplified = simplify_text(clause_data['cleaned_text'])
        ...
        simplified_text = simplify_text(processed_text)
        ...
        simplified_words = len(simplified_text.split()) if simplified_text else int(original_words * 0.7)
        original_sentences = len(sent_tokenize(raw_text))
        simplified_sentences = len(sent_tokenize(simplified_text)) if simplified_text else int(original_sentences * 0.7)
        ...
        stats_data = {
            ...
            'Complex Words': [sum(1 for word in raw_text.split() if syllables.estimate(word) > 2),
                            sum(1 for word in simplified_text.split() if syllables.estimate(word) > 2) if simplified_text else 0]
        }
        ...
        with get_db() as db:
            ...
            db.add(document)
            db.commit()
            db.refresh(document)  # Get generated ID
